...
int TCPServer::HandleClose(SOCKET iSocket)
{
	...
	if(iRet < 0)
	{
		XLOG_ERROR(m_pLogger, "OnClose failed: iRet=%d", iRet);
	}
...
int TCPServer_Input(SOCKET iSocket)
{
	...
	if(iRet < 0)
	{
		XLOG_ERROR(m_pLogger, "ProcessRead error: iRet = %d", iRet);
		return -2;
	}
...
int TCPServer::TCPSend(SocketContext* pstSctx, char* pUserInfo, char* pPkg, int iPkgLen)
{
	...
	if(pContext->iStat != SOCKET_ACCEPT)
	{
		...
		{
			XLOG_ERROR(m_pLogger, "[error] stat (socket %d, stat %d)!", 
	...
	{
		XLOG_ERROR(m_pLogger, "parameter error: %d, %u", iPkgLen, iSendBufSize);
		return -1;
	}
